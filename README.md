...
distribution upgrade, run this in each existing project and replace `0.11.0`
with the installed target version when necessary:

```bash
repofoundry --repo . upgrade --to 0.11.0
repofoundry --repo . upgrade --to 0.11.0 --apply
repofoundry --repo . validate
```
...
repofoundry --repo . upgrade --to 0.11.0
repofoundry --repo . upgrade --to 0.11.0 --governance-profile adaptive
repofoundry --repo . upgrade --to 0.11.0 --apply
...
RepoFoundry `0.11.0` uses Harness schema `3`, Harness Core `1.5.3`, Codex
...
`upgrade --to 0.11.0 --apply`. Earlier schema `3` Core and adapter contracts
