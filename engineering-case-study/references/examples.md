...
读者、分享场景、文章类型、输出语言和中心主张有助于聚焦写作；上下文充分时可以
直接推断，无需用户逐项填写。证据范围可以由用户指定，也可以从目标模块或 EP 反向发现。
...
如果没有说明语言，沿用既有明确偏好或目标文档约定，再默认使用请求语言；例如
没有其他约定的中文请求可以直接生成中文草稿。仅在语言存在影响交付的真实冲突时询问。
